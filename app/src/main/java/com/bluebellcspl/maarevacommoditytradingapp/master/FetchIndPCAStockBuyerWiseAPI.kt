...
class FetchIndPCAStockBuyerWiseAPI(var context: Context, var fragment: Fragment, var model: IndPCAInvoiceStockFragment.IndPCAInvoiceStockFetchAPIModel) {
    val job = Job()
    ...
    private fun getIndPCAInvoiceStockAPI() {
        try {
            ...
            scope.launch(Dispatchers.IO) {
                val result = APICall.getIndPCAInvoiceStockBuyerWiseAPI(JO)
                if (result.isSuccessful) {
                    ...
                    if (mainResponseJO.isJsonObject)
                    {
                        ...
                        val userListType = object : TypeToken<IndPCAStockBuyerWiseModel>() {}.type
                        var indPCAStockFetchList: IndPCAStockBuyerWiseModel = Gson().fromJson(responseJA, userListType)
                        withContext(Dispatchers.Main){
                            commonUIUtility.dismissProgress()
                            if (fragment is IndPCAInvoiceStockFragment){
