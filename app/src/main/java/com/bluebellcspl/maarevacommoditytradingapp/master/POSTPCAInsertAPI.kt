...
//            JO.addProperty("GujaratiShortPCAName",model.GujaratiShortPCAName)
            ...
            val postAddPCAJO=Gson().toJsonTree(model).asJsonObject
            Log.d(TAG, "postPCAData: PCA_INSERT_JSON : $postAddPCAJO")

            val APICall = RetrofitHelper.getInstance().create(OurRetrofit::class.java)

            scope.launch(Dispatchers.IO){
                val result = APICall.postPCAInsertData(postAddPCAJO)
